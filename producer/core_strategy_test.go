...
func TestCoreStrategy_RequestMsg(t *testing.T) {
	t.Run("returns Response with Data and Header and forwards request headers", func(t *testing.T) {
		s, url := runServer()
		defer s.Shutdown()
		...
		strategy := producer.NewCoreStrategy(nc)
		req := strategy.(producer.RequestMsger)
	...
	t.Run("returns nil and error when no responders", func(t *testing.T) {
		...
		resp, err := req.RequestMsg(ctx, &nats.Msg{Subject: "test.reqmsg.no.responders", Data: []byte("ping")})
		assert.Nil(t, resp)
		assert.Error(t, err)
	})
}
